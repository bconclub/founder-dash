...
import { getWindchasersSystemPrompt } from '@/api/prompts/windchasers-prompt';
...
export function buildPrompt({
  ...
  // For PROXe and Windchasers, knowledge base is already in system prompt, so don't duplicate it
  const knowledgeBlock = (isProxe || isWindchasers)
    ? '' // Knowledge base already in system prompt via getProxeSystemPrompt/getWindchasersSystemPrompt

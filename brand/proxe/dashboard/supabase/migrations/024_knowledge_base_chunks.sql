-- Migration 024: Knowledge Base Chunks + pgvector + Hybrid Search
...
CREATE TABLE IF NOT EXISTS knowledge_base_chunks (
  ...
  embedding vector(384),       -- gte-small output dimension (nullable until embeddings generated)
  fts_vector tsvector,         -- pre-computed full-text search vector (auto via trigger)
...
-- Brand filter
CREATE INDEX IF NOT EXISTS idx_kb_chunks_brand
  ON knowledge_base_chunks(brand);
...
-- Works once rows exist; safe to create on empty table
...
DROP POLICY IF EXISTS "Allow all users to view kb_chunks" ON knowledge_base_chunks;
CREATE POLICY "Allow all users to view kb_chunks"
  ON knowledge_base_chunks FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow all users to insert kb_chunks" ON knowledge_base_chunks;
CREATE POLICY "Allow all users to insert kb_chunks"
  ON knowledge_base_chunks FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all users to update kb_chunks" ON knowledge_base_chunks;
CREATE POLICY "Allow all users to update kb_chunks"
  ON knowledge_base_chunks FOR UPDATE USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow all users to delete kb_chunks" ON knowledge_base_chunks;
CREATE POLICY "Allow all users to delete kb_chunks"
  ON knowledge_base_chunks FOR DELETE USING (true);
...
-- ============================================
-- 7. RPC: search_knowledge_base (hybrid full-text + vector)
-- ============================================
-- Called by the web agent's searchKnowledgeBase() function.
-- Signature matches the existing RPC call pattern.
...
BEGIN
  ...
  WITH ranked AS (
    ...
    -- Vector similarity search (only when embedding provided and embeddings exist)
    SELECT
      c.id,
      c.knowledge_base_id,
      c.content,
...
-- ============================================
-- 8. BACKFILL: Migrate existing JSONB chunks into new table
-- ============================================

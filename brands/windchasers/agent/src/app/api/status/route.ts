...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!status.supabaseConfig.urlValid) {
      status.recommendations.push(`${supabaseUrlKey} is missing or invalid. Check your .env.local file.`)
    }
    if (!status.supabaseConfig.anonKeyValid) {
      status.recommendations.push(`${supabaseAnonKeyName} is missing or invalid. Get it from Supabase Dashboard > Settings > API.`)
    }

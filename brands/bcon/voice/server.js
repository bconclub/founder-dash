...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (data) => {
    try {
      ...
      if (msg.event === 'start') {
        callUUID = msg.start?.callId || msg.callId;
        console.log(`Call started: ${callUUID}`);
        await speakToVobiz(ws, "Hello! Welcome to BCON Club. How can I help you today?");
      }
            ...
            if (audioBuffer.length > 10) {
              ...
              try {
                ...
                if (transcript?.trim()) {
                  ...
                  await speakToVobiz(ws, response);
                }
              } catch (err) {
...
async function speakToVobiz(ws, text) {

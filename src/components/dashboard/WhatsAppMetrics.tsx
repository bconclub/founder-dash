...
export default function WhatsAppMetrics() {
  ...
  const [totalMessagesSent, setTotalMessagesSent] = useState<ChartData[]>([])
  const [leadsConverted, setLeadsConverted] = useState<ChartData[]>([])
  ...
  const fetchWhatsAppMetrics = async () => {
    ...
    try {
      ...
      // Fetch WhatsApp messages for time series
      const messagesResponse = await fetch('/api/dashboard/whatsapp/messages')
      const messagesData = messagesResponse.ok ? await messagesResponse.json() : { messages: [] }

      // Fetch WhatsApp leads
      const leadsResponse = await fetch('/api/dashboard/leads?source=whatsapp&limit=1000')
      ...
      // Total Messages Sent (agent messages by date) - daily counts
      const agentMessages = messagesData.messages?.filter((m: any) => m.sender === 'agent') || []
      const messagesDataSeries = generateTimeSeries(
        agentMessages,
        'created_at',
        () => 1,
        false // daily counts, not cumulative
      )
      setTotalMessagesSent(messagesDataSeries)

      // Leads Converted (leads with booking by date) - daily counts
      const convertedLeads = leadsData.leads?.filter((lead: any) => 
        lead.booking_date && lead.booking_time
      ) || []
      const convertedDataSeries = generateTimeSeries(
        convertedLeads,
        'booking_date',
        () => 1,
        false // daily counts, not cumulative
      )
      setLeadsConverted(convertedDataSeries)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <ChartCard
          ...
          value={
            totalMessagesSent.length > 0
              ? totalMessagesSent.reduce((sum, item) => sum + item.value, 0)
              : 0
          }
        ...
        <ChartCard
          ...
          value={
            leadsConverted.length > 0
              ? leadsConverted.reduce((sum, item) => sum + item.value, 0)
              : 0
          }
          data={leadsConverted}
          formatValue={(v) => v.toLocaleString()}
